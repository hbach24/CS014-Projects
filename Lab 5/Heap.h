...
private:
    ...
    /*This function is called by the enqueue function 
    to move a newly inserted item (last item) up the heap
    to its proper location if it violates the max-heap property.
    In other words, if the item is greater than its parents.*/
    void trickleUp(int position);

    /*swaps objects when trying to fulfill max-heap property*/
